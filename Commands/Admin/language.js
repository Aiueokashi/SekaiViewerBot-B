...
class Language extends Command {
  constructor(client) {
    super(client, {
      name: "language",
      description:
        "Change the language used by SekaiViewerBot System Message in the guild",
      usage: "language",
      exemple: [],
      args: false,
      category: "Admin",
  ...
  async run(message) {
    ...
    const selector = await super.respond({
      embed: {
        description: lang[0],
      },
    });
    ...
    const filter = (reaction, user) =>
      flag.includes(reaction.emoji.name) && user.id === message.author.id;

    const collected = await selector.awaitReactions(filter, {
      max: 1,
      time: 60000,
    });
    ...
    message.guild.settings.local = Array.from(client.language.keys())[
      parseInt(index, 10)
    ];
