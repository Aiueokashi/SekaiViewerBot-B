...
class Message {
  ...
  async run(message) {
    ...
    const command = client.commands.get(commandPrefix.toLowerCase()) || client.commands.get(client.aliases.get(commandPrefix.toLowerCase()));
    
    if(!command)
        return;
        
    const lang = client.language.get(message.guild && message.guild.settings.local || "English").message(message,command);
    if (command.cmdCooldown.has(message.author.id))
            return message.delete({ timeout: 10000 })
                && message.reply(lang[0]
                .replace(/<<time>>/gm,(command.cmdCooldown.get(message.author.id) / 1000))
