		...
		this.language = {
			...
			message: (message, cmd) => [
				...
				`You didn't provide any arguments, ${message.author} !`,
				{
					...
					description: `> ${cmd.description.replace('<<p>>',message.guild.settings.prefix)}`,
					fields: [
						{
							name: 'Syntax',
							value: `\`\`\`${cmd.usage}\`\`\``
						},
						{
							...
							value: `\`${(cmd.example &&
								cmd.example
									.map(x => `${message.guild.settings.prefix}${cmd.name} ${x}`)
									.join('\n')) ||
			...
			reload: () => [
				...
				models => `Reloaded all database models : \`${models.length}\`models.`,
				{
					title: `Destroy all cache & reload client extensions : ${
						client.user.tag
					}`,
					fields: [
						{
